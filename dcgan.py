...
class Generator( nn.Module ):
    def __init__( self, d=128 ):
        ...
        self.deconv1 = nn.ConvTranspose2d( opt.latent_dim, d * 8, 4, 1, 0 ,bias=False)
    ...
    def forward( self, input ):
        ...
        x = F.relu( self.deconv1_bn( self.deconv1( x ) ),inplace=True)
        x = F.relu( self.deconv2_bn( self.deconv2( x ) ) ,inplace=True)
        x = F.relu( self.deconv3_bn( self.deconv3( x ) ) ,inplace=True)
        x = F.relu( self.deconv4_bn( self.deconv4( x ) ) ,inplace=True)
...
class Discriminator( nn.Module ):
    ...
    def forward( self, input ):
        ...
        x = F.leaky_relu( self.conv4_bn( self.conv4( x ) ), 0.2 ,inplace=True)
